...
func NewEstateSQLConnectionEnv() *MySQLConnectionEnv {
...
func NewChairSQLConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		Host:     getEnv("MYSQL2_HOST", "mysql2"),
		Port:     getEnv("MYSQL2_PORT", "3306"),
		User:     getEnv("MYSQL2_USER", "isucon"),
		DBName:   getEnv("MYSQL2_DBNAME", "isuumo"),
		Password: getEnv("MYSQL2_PASS", "isucon"),
	}
}
...
func main() {
	...
	var err error
	estateConnectionData = NewEstateSQLConnectionEnv()
	...
	chairConnectionData = NewChairSQLConnectionEnv()
...
func initialize(c echo.Context) error {
	estateSqlDir := filepath.Join("..", "mysql", "db")
	estatePaths := []string{
		filepath.Join(estateSqlDir, "0_Schema.sql"),
		filepath.Join(estateSqlDir, "1_DummyEstateData.sql"),
	}

	chairSqlDir := filepath.Join("..", "mysql2", "db")
	chairPaths := []string{
		filepath.Join(chairSqlDir, "0_Schema.sql"),
		filepath.Join(chairSqlDir, "2_DummyChairData.sql"),
	}

	c.Logger().Debug("initialize estate")
	if err := initializeDB(c, estateConnectionData, estatePaths); err != nil {
		c.Logger().Errorf("failed initialize estate db: %v", err)
		return err
	}
	c.Logger().Debug("initialize chair")
	if err := initializeDB(c, chairConnectionData, chairPaths); err != nil {
		c.Logger().Errorf("failed initialize chair db: %v", err)
		return err
	}
...
func initializeDB(c echo.Context, env *MySQLConnectionEnv, paths []string) error {
...
func postChair(c echo.Context) error {
	...
	insertSql := "INSERT INTO chair(id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock) VALUES "
	var values []string
	...
	_, err = tx.Exec(insertSql+strings.Join(values, ","), params...)
	if err != nil {
...
func postEstate(c echo.Context) error {
	...
	insertSql := "INSERT INTO estate(id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity) VALUES "
	var values []string
	var params []interface{}
	defer tx.Rollback()
	for _, row := range records {
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf("SELECT * FROM estate WHERE ST_Contains(ST_PolygonFromText(%s), point) ORDER BY popularity_desc ASC, id ASC LIMIT %v", coordinates.coordinatesToText(), NazotteLimit)
